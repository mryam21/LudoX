...
import { rollDice, getNewPosition, getMovableTokenIndices, relToAbs, SAFE_ABSOLUTE, getTurnOrderColors, pickRandomToken, timestamp, coinsForRank, } from "./utils/gameLogic.js";
...
const userToGame = new Map(); // socket.id -> game_id
...
function initGameRoom(game_id, players) {
    ...
    const orderedPlayers = colorOrder.map((color) => {
...
function addMessage(room, text, color, type) {
...
function doMoveToken(room, player, tokenIdx) {
        ...
        if (!SAFE_ABSOLUTE.has(absNew)) {
            for (const opponent of room.players) {
                if (opponent.user_id === player.user_id)
                    continue;
                for (let i = 0; i < 4; i++) {
                    if (opponent.tokens[i] < 0 || opponent.tokens[i] > 50)
                        continue;
...
function advanceTurn(room, grantExtra) {
...
async function checkGameOver(room) {
    ...
    if (room.rankings.length < room.players.length - 1)
        return;
...
io.on("connection", (socket) => {
    ...
    socket.on("join_game", async (data, callback) => {
        try {
            ...
            if (game.status !== "waiting")
                return callback({ success: false, error: "Game not accepting players" });
            if (game.players.length >= game.total_players)
    ...
    socket.on("leave_game", async (callback) => {
        try {
            ...
            if (game.players.length === 1) {
                ...
                await Game.findByIdAndUpdate(game_id, { $pull: { players: { user_id: userId } } }, { new: true });
    ...
    socket.on("move_token", (data, callback) => {
        ...
        if (!room.diceRolled || room.diceValue === null)
            return callback({ success: false, error: "Roll first" });
